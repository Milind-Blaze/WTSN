...
def run_simulation_for_lambda(lambda_value, lambda_index, schedule, config, parameters,\
    ...
    # Hardcoding OFDMA parameters
    # TODO: fix this
    delivery_latency_OFDMA = {}
    delivery_latency_OFDMA["MCS 2"] = [816, 1190.4, 1564.8, 1939.2, 2313.6, 2688, 3076.8, 3451.2, 3825.6]
    delivery_latency_OFDMA["MCS 7"] = [542.4, 657.6, 772.8, 888, 1003.2, 1118.4, 1219.2, 1334.4, 1449.6,\
                                    1564.8, 1680, 1795.2, 1896, 2011.2, 2126.4, 2241.6, 2356.8, 2472,\
                                    2587.2, 2688, 2803.2, 2918.4, 3033.6, 3148.8, 3264, 3364.8, 3480,\
                                    3595.2, 3710.4, 3825.6, 3940.8, 4056, 4156.8, 4272, 4387.2, 4502.4]
...
def main():
    ...
    if args.single_process:
        ...
        for i in range(len(lambda_range)):
            results_parallel[i] = run_simulation_for_lambda(lambda_range[i], i, schedule_contention, config, parameters, \
                                                            UEs_directory)
    else:
        with Pool() as pool:
            results_parallel = pool.starmap(run_simulation_for_lambda, \
                                [(lambda_range[i], i, schedule_contention, config, parameters, \
                                    UEs_directory) for i in range(len(lambda_range))]) 
    ...
    if scale == "log":
            plt.yscale('log')
            plt.ylim(10**-2, 10**2)
    plt.plot(np.array(lambda_range[1:])*scaling_factor, slope, ".-")
